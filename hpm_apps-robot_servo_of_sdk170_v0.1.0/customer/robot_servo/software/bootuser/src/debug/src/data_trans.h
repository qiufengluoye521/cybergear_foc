 ...
 * @param[in,out]   pkgUserHdl    user struct to operate on.
 * @returns         0:failed 1:success
 */
uint8_t dealdata_parse(PackageUser_Obj* pkgUserHdl);

/**
 * @brief           group data frame for sending.
 * @param[in,out]   pkgUserHdl    user struct to operate on.
 * @param[in,out]   pkgOutHdl     output struct to operate on.
 */
void dealdata_group(PackageUser_Obj* pkgUserHdl,PackageOut_Obj* pkgOutHdl);
...
/**
 * @brief           write parsed value into para struct
 * @param[in,out]   pkgUserHdl       parsed data  info.
 * @param[in,out]   paraTypHdl       para struct to operate on.
 * @returns         0:failed 1:success
 */
uint8_t data_write_in(PackageUser_Obj* pkgUserHdl,ParaTyp_Obj* paraTypHdl);
/**
 * @brief           read data in para struct out
 * @param[in,out]   pkgUserHdl       parsed data  info.
 * @param[in,out]   paraTypHdl       para struct to operate on.
 * @returns         0:failed 1:success
 */
uint8_t data_read_out(PackageUser_Obj* pkgUserHdl,ParaTyp_Obj* paraTypHdl);

/**
 * @brief           write sequence data in
 * @param[in,out]   pkgUserHdl       parsed data  info.
 * @param[in,out]   pkgOutHdl       para struct to operate on.
 * @returns         1:success
 */
bool seqdata_write_in(PackageUser_Obj* pkgUserHdl,PackageOut_Obj* pkgOutHdl);

/**
 * @brief           read sequence data out
 * @param[in,out]   pkgUserHdl       parsed data  info.
 * @param[in,out]   pkgOutHdl       para struct to operate on.
 * @returns         0:failed 1:success
 */
uint8_t seqdata_read_out(PackageUser_Obj* pkgUserHdl,PackageOut_Obj* pkgOutHdl);

/**
 * @brief           received data deal process
 * @param[in,out]   pkgInfoHdl       package info of processing .
 * @param[in,out]   paraHdl          para struct to operate on.
 * @returns         0:failed 1:success
 */
uint8_t uart_recvdata_deal(PackageInfo_Obj* pkgInfoHdl,Para_Obj* paraHdl);
